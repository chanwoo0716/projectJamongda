$(document).ready(function() {
    $('.refund-link').on('click', function(event) {
        ...
        var bookingNumber = $(this).data('bo_number'); // 예약 번호 가져오기
        var imp_uid = $(this).data('imp_uid'); // 결제 ID 가져오기
        var bo_priceString = $(this).data('bo_price'); // 결제 금액 가져오기
		var bo_price = parseInt(bo_priceString, 10);
		
        console.log('Booking Number:', bookingNumber);
        console.log('Imp UID:', imp_uid);
        console.log('BO Price:', bo_price);
