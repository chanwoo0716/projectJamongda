        ...
        IMP.request_pay({
            ...
            if (rsp.success) {
                ...
                let bookingData = {
                    bo_name: $('#bo_name').val(),
                    bo_tel: $('#bo_tel').val(),
                    bo_checkIn: "2024-07-17", //$('#bo_checkIn').val(),
                    bo_checkOut: "2024-07-18", //$('#bo_checkOut').val(),
                    payDate: new Date(),
                    bo_payment: "카카오페이", //실제로는 선택지를 줘야하지만 현재는 카카오페이 결제만 구현
                    bo_price: 13700, // 결제 금액(예시)
                    ro_id: 1 // 객실 ID (예시)
                };
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        alert("예약이 완료되었습니다.");
                        window.location.href = "/booking/bookingComplete.do?bo_number=" + response.bo_number;
                    },
