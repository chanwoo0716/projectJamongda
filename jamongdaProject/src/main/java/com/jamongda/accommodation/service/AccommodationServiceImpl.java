...
public class AccommodationServiceImpl implements AccommodationService{
	...
	//한 숙소당 객실이 여러개이므로 ro_id가 여러개 나옴 -> ro_id를 리스트에 담아서 리턴해야함-> ro_id는 객실의 개수만큼 생성되어야함->반복문
	@Override
	public List<Integer> addRoom(Map accRoMap) throws DataAccessException{
		
	    int acc_id = (int) accRoMap.get("acc_id");	//숙소id는 외래키이므로 필요함
	    
	    Map<String, List<String>> roMap = (Map<String, List<String>>) accRoMap.get("roMap");	//객실 정보들(객실당 정보들) // roMap: {ro_amenities2=3, ro_amenities1=2, ro_price1=2,...}
	    Map<String, List<String>> roImageMap = (Map<String, List<String>>) accRoMap.get("roImageMap");	//객실 이미지들(객실당 이미지들) // roImageMap={ro_image1=[hwangsungbin.webp,..], ro_image2=[hwangsungbin3.webp,...]}
	    
		List<Integer> roIdList = new ArrayList<>();	// 객실 ID들 넣는 리스트
		
		//*채워져야할 내용*
		 for (Map.Entry<String, List<String>> entry : roMap.entrySet()) {
	       	String roomName = entry.getKey(); // 객실 이름 (room1, room2, ...)
	        List<String> roomAttributes = entry.getValue(); // 객실 속성들 [ro_name, ro_min, ro_max, ro_info, ro_amenities, ro_price, ro_checkIn, ro_checkOut]
		...
		//
		System.out.println(roIdList);
		
		return roIdList;
	}
	
	// 숙소/객실관리 (숙소/객실 정보, 숙소/객실 이미지, regCheck 가져오기)
	// getRegCheck(), selectAllAcc(), selectAllRo(), selectAccImages(), selectRoImages()
	@Override
